...
handles.exepath=cd; %记住exe文件�?1?7在的文件�?1?7
handles.execd=cd;  %记录exe文件�?1?7在的目录
handles.tempcd=cd;  %用来记录上一个打�?1?7的目录位置，方便文件读写
handles.saveresult = cd; % remember the current dir to save the result

set(handles.text21, 'visible', 'off');   % aper的mrad�?1?7nm单位
set(handles.text125, 'visible', 'on');
set(handles.text24, 'visible', 'on');   % convergence & aperture的mrad�?1?7nm单位
...
handles.conv_sampling=1; %把卷积的点源尺寸，以及放大率抽样�?1?7?1?7�初值不进行任何操作

set(handles.GPURB,'value',1);  %默认GPU计算


%把批量生图的�?1?7些信息存下来，设定初�?1?7
%文件路径
...
cd(handles.tempcd)  %进入上一次打�?1?7的目�?1?7
[FileName,PathName]=uigetfile({'*.pdb','Program Database File (*.pdb)'});%打开文件
if length(FileName)==1 & FileName(1)==0;  %如果选择cancal没打�?1?7文件的话，程序终�?1?7
    cd(handles.execd)
    return;
end
set(handles.edit1,'string',FileName);

cd(handles.execd);  %回到旧的文件�?1?7
handles.tempcd=PathName;  %记录上一个打�?1?7的文件夹

%读取pdb或�?1?7�cif的具体内�?1?7
...
handles.x(:,5) = 0;  %价�?1?7?1?7
handles.x(:,6) = 0;  %离子�?1?7
...
atompos(:,3) = atompos(:,3)-min(atompos(:,3)); %+1.5*eachthick;  % 原子高度，最小�?1?7�变�?1?7�且增加�?1?7层真空层厚度，方便分�?1?7
                                                            % 并且保证不会位于�?1?7��?1?7�因为这里最少是位于第二层，下两行取了floor的行数�?1?7?1?7
                                                            % 比如厚度�?1?70-3-6-9；加�?1?75*eachslice后，值为4.5。分在第�?1?7层正中间

atompos(:,1) = atompos(:,1)-min(atompos(:,1)); %+handles.extendis;
atompos(:,2) = atompos(:,2)-min(atompos(:,2)); %+handles.extendis;

handles.atompos = atompos;  %重新修改�?1?7下原子的坐标，其他信息需要查看handles.x里面读取的结果；
                %只有handles.x中的坐标�?1?7要改变一下；只有在设置层厚时候；
handles.x(:,2:4)=atompos;  %重新给原子的坐标，保证都是大�?1?7�，且有边界
%%%%%%%%%%%%%%%%%%%%%%%-----------------end 202012290930
...
%---begin 202012290935
handles.x_new=handles.x;  %把旋转前的结构记录下来；handles.x_new记录的是旋转�?1?7
...
%�?1?7要包含比较多的信息，但是现在这个pdb信息较少,第一列是元素质子数，二到四是坐标，第五列占据率，第六列假装是价�?1?7?1?7
...
item_ele=unique(handles.x(:,1));  %考察有几种原�?1?7
for i=1:length(item_ele)  %元素的种�?1?7 there are many elements
    r=1-mod(item_ele(i),5)*0.25;    %根据原子序数，设置该原子特有的颜色，r,g,b为三基色的�?1?7?1?7
    g=1-mod(floor(item_ele(i)/5),5)*0.25;
    b=1-mod(floor(item_ele(i)/25),5)*0.25;
    %居中画图
    axes(handles.axes1);  %画原�?1?7 draw atoms
    plot3(atompos(find(handles.x(:,1)==item_ele(i)),1),...
        atompos(handles.x(:,1)==item_ele(i),2), ...
        atompos(find(handles.x(:,1)==item_ele(i)),3),'o',...  %绘点，形状为‘o�?1?7
         'MarkerEdgeColor',[0 0 0],...    %设置边缘颜色为黑�?1?7
         'MarkerSize',handles.dis_atomsize.*(20-log2(2)),...  %设置原子的大�?1?7,随着显示cell数目的增加大小会变小
         'MarkerFaceColor',[r g b]);    %设置原子的颜�?1?7
     axes(handles.axes2);  hold on;%画图显示�?1?7下图�?1?7 draw atoms' label
     plot(0.1*i,0.7,'o',...     %显示原子
     'MarkerEdgeColor',[0 0 0],...
     'MarkerSize',15,...
     'MarkerFaceColor',[r g b]);
     text(0.1*i-0.01,0.3,num2str(item_ele(i)));  %在原子下方显示对应的原子质子�?1?7
end
axes(handles.axes1);  %画原�?1?7
axis equal
xlabel('x'); ylabel('y'); zlabel('z');
axes(handles.axes2);
box off;axis off;   %不显示边框和坐标�?1?7
xlim([0 1]);ylim([0 1]);  %设置x轴与y轴的取�?1?7�范�?1?7
guidata(hObject, handles);
...
cd(handles.tempcd)  %进入上一次打�?1?7的目�?1?7
[FileName,PathName]=uigetfile({'*.xls','Excel File (*.xls)';'*.xlsx','Excel File (*.xlsx)';'*.txt','TEXT File (*.txt)'});%打开文件
if length(FileName)==1 & FileName(1)==0;  %如果选择cancal没打�?1?7文件的话，程序终�?1?7
    cd(handles.execd)
    return;
end
...
if get(handles.popupmenu5,'value') == 3
    set(handles.popupmenu5,'value',2);  %如果�?1?7要分层计算，但是又旋转了lobato的系数；�?1?7要强行换为peng�?1?7
end
guidata(hObject, handles);
...
%----------------------------------------
function resul=Gaussian_focal(mtotal,rms,lambda,gmax);   %计算高斯分布的�?1?7?1?7
M=(mtotal-1)/2;
if M==0
    resul.delta_yita=0;  %离焦偏离是多�?1?7
    resul.gfsf=1;      %高斯取�?1?7�是多少
else
    resul.delta_yita=energyspread(lambda, mtotal, rms, gmax);  %认为单张图由多张图叠加，确定离焦间距的具体数�?1?7
    resul.gfsf=gaussian_focal(mtotal,rms,resul.delta_yita);   %确定�?1?7优离焦条件下，高斯离散的百分比比�?1?7 
end
...
function delta_yita=energyspread(lambda, mtotal, rms, gmax)   %计算�?1?7优的deltayita的�?1?7�，即离焦偏离是多少
M=(mtotal-1)/2;
...
for i=1:length(tempdelta)
    ...
    f_delta_value(i)=max( abs( fixvalue-real( ( gfsf*exp( -sqrt( -1 ) *pi*lambda*( ( -M:M )' )*tempdelta(i)*uu.^2) ) ) ) );   %相减之后的差�?1?7,注意这里在论文里面有录入错误，程序是正确�?1?7
end
...
function tccpara=readtccfromhandles_newSTEM(hObject, handles, lambda)  %本次是根据TEM的参数来读取�?1?7
tccpara.lambda=lambda;
%if get(handles.checkbox_polar,'Value')==1  %必定是极坐标�?1?7
   %读取说明�?1?7%参�?1?7�Phil. Trans. R. Soc. A 2009, vol
%367:3755-3771;但是圈出来的系数在UM  1998 72 PP109-119以及UM 1996 64 249-264中，B2 S3 D4 B4
%S5 R5(D5)，并没有系数，因此，读取数据后，应该要分别乘�?1?7 3  4  5  5  6  6
...
tccpara.A1=str2num(get(handles.edit_A1_1,'String')); tccpara.phiA1=-str2num(get(handles.edit_A1_2,'String'));
   ...
   tccpara.B2=str2num(get(handles.edit_B2_1,'String')); %这个参数引入时�?1?7�和rew相差�?1?7��?1?7�例如rew里面�?1?700，这里必须是6000；因为程序中的系数多乘了1/3
   tccpara.phiB2=-str2num(get(handles.edit_B2_2,'String'));  
  
   %Cs的单位从um到nm
   tccpara.Cs=str2num(get(handles.edit_Cs,'String'))*(10^3); %UNIT from um to nm
   
   %A3的单位从um到nm；A3的角度与电镜显示的除�?1?7
   ...
   %S5的单位从mm到nm�?1?7 %这个数�?1?7�在hrtem里面还没有出现，因此角度关系暂时没有�?1?7
   tccpara.S5=str2num(get(handles.edit_S5_1,'String'))*(10^6); tccpara.phiS5=-str2num(get(handles.edit_S5_2,'String'));
  
   %D5的单位从mm到nm�?1?7%这个数�?1?7�在hrtem里面还没有出现，因此角度关系暂时没有�?1?7
...
%sx、sy为两个方向�?1?7�易空间单位波矢，例�?1?7** �?1?7-1；kvector1为光阑的波矢长度低频，kvector2为波矢高频，例如10 �?1?7-1；shiftx为平移的波矢，例如从中心处移动一点点,flag表示操作方式，取kvector1到kvector2之间的信息，还是不取这之间的信息�?1?7
...
%if paraflag == 'p'|| paraflag == 'l'  %彭�?1?7�师公式 20210223
    [all_nuclear, all_nuc_ion, absorp_n, absorp_n_i, corr_peng_nuc, corr_peng_nuc_ion]=CommonPart_forsimulation_lobato_peng(hObject, eventdata, handles,paraflag);  %step0-step3移动到这�?1?7
%elseif paraflag == 'n' %彭�?1?7�师公式加修�?1?7
     %如果是peng的修正，会得到corr相关的两个矩阵�?1?7�计算�?1?7�路，是把这两个矩阵计算势场，之后作为原势场的初�?1?7
%end
    
%外层循环是所有原子的坐标随机变化30�?1?7
%内层循环：是否�?1?7�虑分层效应，如果是，则�?1?7要多搭几层，再计算每层的贡献�?1?7

%all_nuclear 第一列是元素种类，第2-4列是坐标，第5列是B因子，第6列是占据�?1?7
%step 4
%提取�?1?7些常数，与电压�?1?7�抽样率无关
h_ba=(6.6255916e-34)/2/pi;
h=6.6255916e-34;
e=1.602102e-19;
me=9.1093807e-31;
...
%step 5
%修改2020.04.29
if get(handles.radiobutton11,'value') ||  get(handles.radiobutton10,'value') ||  get(handles.radiobutton13,'value')  %stem�?1?7 or idpc or CBED�?1?7 
   probesx=str2num(get(handles.edit4,'String'));
   [para_part1, U, V]=CommonPara_TEM(handles, probesx, probesx);  %画CTF和PhasePlate用固定的大小即可  
   para_part2 = readtccfromhandles_newTEM(hObject, handles, para_part1.lambda);   %只用�?1?7个函数来把框格中的数据都读取了，这样保证其他地方�?1?7要读取数据时候，用统�?1?7的代�?1?7
    %参数分为两个部分，lambda�?1?7要都�?1?7
   ...
   myap=ones(probesx);  %构�?1?7�光阑函数，kmax是外面定�?1?7
   myap=myaperture(myap,1/(para_part1.sampling.*probesx),1/(para_part1.sampling.*probesx),0,para_part1.gmax*0.01/para_part1.lambda,0,0,1); %如果10mrad，读入就�?1?7�再乘以0.01，换到到rad，再除以nm单位的波�?1?7
   num=length(mytcc(1,1,:));  %考察有多少个probe�?1?7要扫过图�?1?7
   for i=1:num  %光阑在中间，mytcc的重心移边上了�?1?7?1?7
        guiyihua=sum(sum(abs(ifft2(ifftshift(myap.*mytcc(:,:,i)))).^2));  %20201226 add one aper to normalize 
        guiyihua=sqrt(guiyihua);  %20201231�?1?7要求�?1?7根号，才能让probe满足3.59�?1?768�?1?7
        handles.probe(:,:,i) =  fftshift(ifft2(ifftshift(myap.*mytcc(:,:,i))))./guiyihua;
   end
   handles.gfsf=para_part1.gfsf;
end
%__________修改2020.04.29 主要删除原有的STEM后缀的那些程�?1?7
if get(handles.radiobutton9,'value')  %HRTEM代码  翻译代码
    sy=handles.green_Nrow;sx=handles.green_Ncol;   %绿色框格的尺�?1?7
...
for vib = 1 : allvib;  %vibration
    %随机产生原子的坐标偏�?1?7,�?1?7要与每个原子的Debye有关；之后叠加到原子的坐标上
    ...
    if ~isempty(all_nuclear) & get(handles.radiobutton2, 'value') % if vibration is required
        ...
        if paraflag == 'n'
            if ~isempty(corr_peng_nuc)  %如果�?1?7要修正势场，把修正部分的参数的原子位置更新一�?1?7
                corr_nuclear_copy = corr_peng_nuc;
    ...
    if ~isempty(all_nuc_ion) & get(handles.radiobutton2, 'value')
        ...
        if paraflag == 'n'
            if ~isempty(corr_peng_nuc_ion) %如果�?1?7要修正势场，把修正部分的参数的原子位置更新一�?1?7
                corr_peng_nuc_ion_copy = corr_peng_nuc_ion;
    ...
    %�?1?7要或者不�?1?7要计算高度带来的差别,并且还需要清除没有落在范围内的原子�?1?7?1?7    ~~~~~~~~~~~~~~~~~~~~~~~
    %improved multi-slice method
    flag=get(handles.checkbox1, 'value');  %是否�?1?7要�?1?7�虑原子划分在不同的�?1?7
    if flag==1
...
%              CalAllEquilent20201106(all_nuclear_copy, all_nuc_ion_copy, absorp_n, absorp_n_i, eachthick, slicethick, flag);  
    DBmode=double(get(handles.radiobutton4,'value')|get(handles.radiobutton14,'value'));  %任何�?1?7个数�?1?7�就�?1?7
    [ele_n, ele_n_i, absorp_n, absorp_n_i, series_n, series_n_i] = ...
    ...
    %得到第vib张图�?1?7  ~~~~~~~~~~~~~~~~~~~~
    if vib==1
         if get(handles.radiobutton11,'value') || get(handles.radiobutton13,'value')  %stem�?1?7
             [myresul, midresul] = STEMsimulation(handles, ele_n, ele_n_i, absorp_n, absorp_n_i, series_n, series_n_i, ele_n_corr, ele_n_i_corr, corr_info, ... %增加修正peng的系数，带入的修正势场的�?1?7
    series_n_corr, series_n_i_corr);
         end
         if get(handles.radiobutton10,'value')  %cbed�?1?7, 
             myresul=CBEDsimulation(handles, ele_n, ele_n_i, absorp_n, absorp_n_i, series_n, series_n_i, ele_n_corr, ele_n_i_corr, corr_info, ... %增加修正peng的系数，带入的修正势场的�?1?7
    series_n_corr, series_n_i_corr);
         end
         if get(handles.radiobutton9,'value')  %hrtem�?1?7
             myresul=HRTEMsimulation(handles, ele_n, ele_n_i, absorp_n, absorp_n_i, series_n, series_n_i, mytcc, ele_n_corr, ele_n_i_corr, corr_info, ... %增加修正peng的系数，带入的修正势场的�?1?7
    series_n_corr, series_n_i_corr);
         ...
         vib
         if get(handles.radiobutton11,'value') || get(handles.radiobutton13,'value')  %stem�?1?7
             [myresultemp, midresultemp] = STEMsimulation(handles, ele_n, ele_n_i, absorp_n, absorp_n_i, series_n, series_n_i, ele_n_corr, ele_n_i_corr, corr_info, ... %增加修正peng的系数，带入的修正势场的�?1?7
    series_n_corr, series_n_i_corr);
             myresul=myresul+myresultemp;
             midresul = midresul+midresultemp; 
         end
         if get(handles.radiobutton10,'value')  %cbed�?1?7
             myresul=myresul+CBEDsimulation(handles, ele_n, ele_n_i, absorp_n, absorp_n_i, series_n, series_n_i,ele_n_corr, ele_n_i_corr, corr_info, ... %增加修正peng的系数，带入的修正势场的�?1?7
    series_n_corr, series_n_i_corr);
         end
         if get(handles.radiobutton9,'value')  %hrtem�?1?7
...
allstemnum = 1; %�?1?7有stem图像的个�?1?7
steminfo = [];
if get(handles.radiobutton11,'value')  %stem�?1?7 �?1?7要重新reshape矩阵大小
    aper_dect = str2num(get(handles.edit79,'string'));%制�?1?7�接收光�?1?7 
...
if get(handles.radiobutton9,'value')  %hrtem�?1?7 �?1?7要重新reshape矩阵大小
...
if get(handles.radiobutton10,'value')  %CBED�?1?7 �?1?7要重新reshape矩阵大小
    for i=1:length(myresul(1,1,:))
...
%Kepeng
if get(handles.radiobutton13,'value')  %idpc�?1?7
    aper_dect = str2num(get(handles.edit79,'string'));%制�?1?7�接收光�?1?7 
    ...
    k2=kx.^2+ky.^2;  %数�?1?7�范围不�?1?7样，要检�?1?7
...
for i=1:num  %构建接收光阑
    ...
    if aper_dect(i,3) >180  %�?1?7360的书写范围转变到-180 �?1?70
    ...
    APER(:,:,4*(i-1)+1) = temp.* APER(:,:,4*(i-1)+1);  %加上带�?1?7�了
...
function myresul=HRTEMsimulation(handles, ele_n, ele_n_i, absorp_n, absorp_n_i, series_n, series_n_i, mytcc,...
    ele_n_corr, ele_n_i_corr, corr_info, ... %增加修正peng的系数，带入的修正势场的�?1?7
    series_n_corr, series_n_i_corr)
%�?1?7要计算green的尺寸，以及�?1?7有原子的相对位置，计算的区域，以及拼凑回原图时�?1?7�的坐标
h_ba=(6.6255916e-34)/2/pi;
h=6.6255916e-34;
e=1.602102e-19;
me=9.1093807e-31;
...
sigma=2*pi/(handles.lambda*10*handles.vol/1000)*(me*c*c+e*handles.vol)/(2*me*c*c+e*handles.vol);  %单位与书�?1?7样，/kV*A
...
%重大改进�?1?7210225，否则高频的晶格信息丢失�?1?7
clear tx ty

paraflag = getparaflag(handles);
if get(handles.GPURB,'value')  %使用GPU来计�?1?7
    %GPU必须是方�?1?7
    %%绿色的尺寸，绿色倒格矢�?1?7�gx用于计算原子位置的，s2使用peng的参数计算原子势场分�?1?7
    ...
    [gx_green,gy_green]=meshgrid((-green/2:(green/2-1))./(green*handles.sampling), ...
        (-green/2:(green/2-1))./(green*handles.sampling)); %REPRO单位�?1?7A)^2,不是(1/nm)^2
    ...
    if paraflag=='n'
        ...
        corr_info_matrix=0.*s2_green;%赋初�?1?7
        for i = 1:length(corr_info(:,1))  %把每种的修正势场都算出来，种类很少，�?1?7以算与元素个数相当的矩阵就好
            r2 = s2_green;
    ...
    for i=1:length(potential_temp(1,1,:)); %�?1?7要转�?1?7
...
[gx_green,gy_green]=meshgrid((-handles.green_Nrow/2:(handles.green_Nrow/2-1))./(handles.green_Nrow*handles.sampling), ...
        (-handles.green_Ncol/2:(handles.green_Ncol/2-1))./(handles.green_Ncol*handles.sampling)); %REPRO单位�?1?7A)^2,不是(1/nm)^2
    sx_green=gx_green/2;  % peng 的s参数
    sy_green=gy_green/2;
    s2_green=sx_green.^2+sy_green.^2;
    

if get(handles.CPURB,'value')  %使用CPU来计�?1?7 
    
%计算每层的势�?1?7
% potential=GetPotential4AllSlice_multicore_lobato_peng(handles.green_Ncol, handles.green_Nrow,... 
%     ele_n, absorp_n, ....仅有离子或�?1?7�原子的弹�?1?7�和吸收
%     ele_n_i, absorp_n_i, ... %原子或原�?1?7+离子，弹性或者吸�?1?7
%     series_n, series_n_i, ...  %原子排列次序   
%     s2_green, gx_green, gy_green, ...
%     sigma, PARAMETER, APERTURE, paraflag);   %为了STEM计算不出错，这里只带入到HRTEM和CBED�?1?7
APERTURE=ones(handles.green_Ncol, handles.green_Nrow);
potential=GetPotential4AllSlice_multicore_lobato_peng_corr(handles.green_Ncol, handles.green_Nrow,... 
    ele_n, absorp_n, ....仅有离子或�?1?7�原子的弹�?1?7�和吸收  %本函数调用修正peng的参�?1?7
    ele_n_i, absorp_n_i, ... %原子或原�?1?7+离子，弹性或者吸�?1?7
    series_n, series_n_i, ...  %原子排列次序   
    ele_n_corr, ele_n_i_corr, corr_info, ... %如果修正peng的系数，带入的修正势场的�?1?7
    series_n_corr, series_n_i_corr,...
    s2_green, gx_green, gy_green, ...
    sigma, PARAMETER, APERTURE, paraflag);   %为了STEM计算不出错，这里只带入到HRTEM和CBED�?1?7
end
...
tiltx=tilt/lambda*cos(phitilt/180*pi); %换算到�?1?7�易格式的单�?1?7 1/nm
...
%波函数传�?1?7  %20210119 入射倾斜的波函数----------------

%   波函数propogation;与zheight有关
p2=((gx_green+tiltx*0.1).^2+(gy_green+tilty*0.1).^2);  %传播�?1?7
psf_fft=exp(-sqrt(-1)*pi*handles.lambda*10*handles.eachthick*p2);   %都在单位A计算�?1?7
psf_fft(find(p2>1/(16*handles.sampling*handles.sampling)))=0;  %加入半光�?1?7
...
if tilt > 0
     ...
     myfftwave(mycen(handles.green_Ncol), mycen(handles.green_Nrow)) = 1;  %赋�?1?7�为平面�?1?7
end
   %this wave shift is the same as the U*U+V*V before about line 885 U=U-para_part1.tiltx;   
mywave=ifft2(ifftshift(myfftwave))*handles.green_Nrow*handles.green_Ncol;  %读入波函�?1?7
%----------end 20210119
...
for k=1:length(potential(1,1,:));
    if rem(k,10)==0
        ...
        mywave=fft2(mywave);   %注意，节省掉�?1?7个fftshift
...
function myresul=CBEDsimulation(handles, ele_n, ele_n_i, absorp_n, absorp_n_i, series_n, series_n_i, ele_n_corr, ele_n_i_corr, corr_info, ... %增加修正peng的系数，带入的修正势场的�?1?7
    series_n_corr, series_n_i_corr);
%�?1?7要计算green的尺寸，以及�?1?7有原子的相对位置，计算的区域，以及拼凑回原图时�?1?7�的坐标
h_ba=(6.6255916e-34)/2/pi;
h=6.6255916e-34;
...
p2_probe=(px.^2+py.^2);  %传播�?1?7
psf_fft=exp(-sqrt(-1)*pi*handles.lambda*10*handles.eachthick*p2_probe);   %都在单位A计算�?1?7
%psf_fft(find(p2_probe>1/(16*handles.sampling*handles.sampling)))=0;  %加入半光�?1?7
psf_fft(find(p2_probe>1/(9*handles.sampling*handles.sampling)))=0;  %加入2/3的光�?1?7
psf_fft=ifftshift(psf_fft);
...
%     series_n, series_n_i, ...  %原子排列次序
%     s2_green, gx_green, gy_green, ...
%     sigma, PARAMETER, APERTURE);   %为了STEM计算不出错，这里只带入到HRTEM和CBED�?1?7

%成像光阑
myap=ones(handles.CBEDprobesx);
myap(find(p2_probe>1/(9*handles.sampling*handles.sampling)))=0;  %16是加入半光阑 %9是加�?1?73的光�?1?7

%传播
myresul=zeros(handles.CBEDprobesx);
[probesx, probesy]=size(handles.probe);
for nn=1:length(handles.gfsf);
    mywave=handles.probe(:,:,nn);  %读入波函�?1?7
    for k=1:length(potential(1,1,:));
...
%myresul=myresul.*myap;  %加上光阑�?1?7
return;

 

        
function [myresul,mid_ceng_mat] = STEMsimulation(handles, ele_n, ele_n_i, absorp_n, absorp_n_i, series_n, series_n_i, ele_n_corr, ele_n_i_corr, corr_info, ... %增加修正peng的系数，带入的修正势场的�?1?7
    series_n_corr, series_n_i_corr);
%�?1?7要计算green的尺寸，以及�?1?7有原子的相对位置，计算的区域，以及拼凑回原图时�?1?7�的坐标
h_ba=(6.6255916e-34)/2/pi;
h=6.6255916e-34;
...
; %换算到�?1?7�易格式的单�?1?7 1/nm
tilty=tilt/lambda*sin(phitilt/180*pi);

%20210119 入射倾斜的波函数在多层法传播时的注意�?1?7----------------
%   波函数propogation;与zheight有关
[px, py]=meshgrid( (-handles.probesx/2:(handles.probesx/2-1))./(handles.probesx*handles.sampling), ...
    (-handles.probesx/2:(handles.probesx/2-1))./(handles.probesx*handles.sampling));
p2_probe=((px+tiltx*0.1).^2+(py+tilty*0.1).^2);  %传播�?1?7
psf_fft=exp(-sqrt(-1)*pi*handles.lambda*10*handles.eachthick*p2_probe);   %都在单位A计算�?1?7
psf_fft(find(p2_probe>1/(16*handles.sampling*handles.sampling)))=0;  %加入半光�?1?7
% psf_fft(find(p2_probe>1/(9*handles.sampling*handles.sampling)))=0;  %加入2/3的光�?1?7
psf_fft=ifftshift(psf_fft);
...
%Kepeng
if get(handles.radiobutton11,'value')  %stem�?1?7
    num=length(aper_dect(:,1));
    APER=zeros(handles.probesx, handles.probesx,num);
    for i=1:num  %构建接收光阑
        temp=zeros(handles.probesx, handles.probesx);
...
%         end  %gpu,输入这个矩阵更方便一�?1?7
    ...
    %cuda计算，需要输入的变量�?1?7 corr_info_matrix; ele_n_corr, ele_n_i_corr;series_n_corr, series_n_i_corr
    tic
    cuda_stem;
    toc
end
...
if get(handles.radiobutton11, 'value') | get(handles.radiobutton13, 'value')   %如果是STEM和IDPC的计算，显示相应的区�?1?7
    handles.sampling=str2num(get(handles.edit75,'string'));  %�?1?7/pixel   %这个是传播矩阵和投影势函数矩阵的抽样率，不是probe的扫描抽样率
    if ~isempty ( strfind(get(handles.edit77,'string'), '.'))
        msgbox('''Scan Step'' Must be an integer');
        return;
    end
    handles.probestepsampling=str2num(get(handles.edit77,'string')) * handles.sampling;  %表示扫描时�?1?7�的分辨�?1?7
    handles.probesx=str2num(get(handles.edit4,'string'));  %读取probe的尺�?1?7
    if rem(handles.probesx,2)==1 handles.probesx=handles.probesx+1;end
    
    %设定扫描区域topleft-rightdown的坐�?1?7
        %红色框的宽度和高度分别为多少像素
    ...
    %左上角和右下角的坐标（x，y）�?1?7��?1?7�单位为anstrong。笔记中的红色框

    %放在�?1?7始的时�?1?7�赋初�?1?7�outside_ext=4;  %再向外延�?1?7�，这样保证投影势场做傅里叶变换时�?1?7�，周期延拓的效果不会影响probe扫描区域�?1?7
                %笔记中的绿色框，单位也是�?1?7
    tl_green=tl_red-handles.sampling*round(handles.outside_ext./handles.sampling)-handles.probesx/2*handles.sampling;  %第二个减法的操作，是为了保证在扫描时，所选的区域左上角坐标是整数个像�?1?7
    rd_green=rd_red+handles.sampling*round(handles.outside_ext./handles.sampling)+handles.probesx/2*handles.sampling;  %第二个加法的操作，是为了保证在扫描时，所选的区域左上角坐标是整数个像�?1?7

    greensize = round((rd_green - tl_green)./handles.sampling);  
    if rem(greensize(1),2)==1 greensize(1)=greensize(1)+1;end %绿色区域的图像尺寸，单位是像�?1?7
    ...
    %�?1?7要存�?1?7下绿色框的像素数，抽样率；probe的尺寸；红色框的宽和高�?1?7�~~~~~~~~~~~~~
    load allresul
    atompos= allresul.x_new(:,2:4);  %重新修改�?1?7下原子的坐标，其他信息需要查看handles.x里面读取的结果；

    handles.green_Nrow=greensize(1);
    handles.green_Ncol=greensize(2);  %行和�?1?7
    handles.probe_ingreenNrow=probe_in_green(1);  
    handles.probe_ingreenNcol=probe_in_green(2);   %在绿色框中�?1?7�取的坐标位�?1?7
    handles.width_red=width_red; 
    handles.hight_red=hight_red;  %扫描区的像素个数
    handles.probestep=str2num(get(handles.edit77,'string')); %扫描时的步长(单位是�?1?7�数�?1?7
    handles.tl_green=tl_green;   %把绿色框的左上角坐标（埃单位）保存下来，�?1?7要之后换算所有的原子位置，并做转�?1?7
    handles.rd_green=tl_green+greensize*handles.sampling;%绿色框的右下角坐标，�?1?7要�?1?7�虑哪些原子在这个范围外的，就不带入stem成像的后续计算�?1?7?1?7
    ...
    hold on; %plot(tl_red(1), tl_red(2), 'ko');plot(rd_red(1), rd_red(2), 'ko')
         ...
         %画一个probe的形状，这个probe的中心位于红色区域的左上角坐�?1?7
...
if get(handles.radiobutton10, 'value')  %如果是CBED�?1?7
    handles.sampling=str2num(get(handles.edit75,'string'));  %�?1?7/pixel   %这个是传播矩阵和投影势函数矩阵的抽样率，不是probe的扫描抽样率
    ...
    tl_red=[str2num(get(handles.edit17,'string')), str2num(get(handles.edit18,'string'))]; 
    %红色的是作图区域，这里是probe的尺�?1?7
    rd_red=tl_red+handles.CBEDprobesx*handles.sampling; 

    tl_green=tl_red-handles.sampling*round(handles.outside_ext./handles.sampling);  %埃单�?1?7  %绿色的左上角和右下角坐标。图像需要扩展一定尺寸，否则会有wrap
    rd_green=rd_red+handles.sampling*round(handles.outside_ext./handles.sampling);  

    greensize = round((rd_green - tl_green)./handles.sampling);  
    if rem(greensize(1),2)==1 greensize(1)=greensize(1)+1;end %绿色区域的图像尺寸，单位是像�?1?7
    if rem(greensize(2),2)==1 greensize(2)=greensize(2)+1;end
    %给出imaging位于绿色框内的起始坐标点位置;之后从大图中裁剪�?1?7终成像结�?1?7
    ...
    handles.green_Nrow=greensize(1);
    handles.green_Ncol=greensize(2);  %行和�?1?7
    handles.CBED_ingreenNrow=imaging_in_green(1); %注意已经+1了，�?1?7以从这点�?1?7始直接取位置 
    handles.CBED_ingreenNcol=imaging_in_green(2);   %在绿色框中�?1?7�取的坐标位�?1?7
    handles.tl_green=tl_green;   %把绿色框的左上角坐标（埃单位）保存下来，�?1?7要之后换算所有的原子位置，并做转�?1?7
    handles.rd_green=tl_green+greensize*handles.sampling;%绿色框的右下角坐标，�?1?7要�?1?7�虑哪些原子在这个范围外的，就不带入stem成像的后续计算�?1?7?1?7
...
if get(handles.radiobutton9, 'value')  %如果是HRTEM的计算，显示相应的区�?1?7
    handles.sampling=str2num(get(handles.edit75,'string'));  %�?1?7/pixel   %这个是传播矩阵和投影势函数矩阵的抽样率，不是probe的扫描抽样率
    handles.HRTEMedgesx=str2num(get(handles.edit4,'string'));  %读取外围有多大区域需要裁�?1?7

    %设定成像区域topleft-rightdown的坐�?1?7
    %红色框的宽度和高度分别为多少像素
    width_red=str2num(get(handles.edit20,'string')); hight_red=str2num(get(handles.edit19,'string'));
    
    tl_red=[str2num(get(handles.edit17,'string')), str2num(get(handles.edit18,'string'))]; 
    rd_red=tl_red+[width_red, hight_red]*handles.sampling;   %单位是anstrong  %这里与STEM不同
    %左上角和右下角的坐标（x，y）�?1?7��?1?7�单位为anstrong�?1?7

    %绿色框，单位也是埃；是实际成像时的左上角和右下角坐标，之后要扣掉剪裁区；并且这个是边界绝对的尺寸，不�?1?7要除�?1?7�也就是说，扣掉256的话，成像时候�?1?7�择的大小至少要512
    tl_green=tl_red-handles.HRTEMedgesx*handles.sampling;  %埃单�?1?7
    ...
    x=handles.x;
    atompos=x(:,2:4);  % 单位变埃~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    %�?1?7要换算出，所有的原子位置都放在绿色的框里面的起始位置是什�?1?7
...
%     handles.eachthick=eachthick;  %记录厚度
    handles.green_Nrow=greensize(1);
    handles.green_Ncol=greensize(2);  %行和�?1?7
    handles.HRTEM_ingreenNrow=imaging_in_green(1); %注意已经+1了，�?1?7以从这点�?1?7始直接取位置 
    handles.HRTEM_ingreenNcol=imaging_in_green(2);   %在绿色框中�?1?7�取的坐标位�?1?7
    handles.tl_green=tl_green;   %把绿色框的左上角坐标（埃单位）保存下来，�?1?7要之后换算所有的原子位置，并做转�?1?7
    handles.rd_green=tl_green+greensize*handles.sampling;%绿色框的右下角坐标，�?1?7要�?1?7�虑哪些原子在这个范围外的，就不带入stem成像的后续计算�?1?7?1?7
    handles.width_red=width_red;
...
%求波长，求mrad，以确定�?1?7小的sampling rate应该是多�?1?7
h_ba=(6.6255916e-34)/2/pi;
h=6.6255916e-34;
e=1.602102e-19;
me=9.1093807e-31;
...
aper_dect = str2num(get(handles.edit79,'string'));  %detector 的光阑尺�?1?7
kmax=max(aper_dect(:))*10^(-3);

kk=kmax./(lambda*10);  %�?1?7高的倒空间的频率是多少，单位�?1?7�?1?7

%由于正空间的�?1?7小间隔单位为 sampling，所以最高的倒空间频率为1/sampling�?1?7
%另外，由于s的最大有效�?1?7�为6 埃分之一值；划到g的�?1?7�的话，�?1?7s=12 埃分之一�?1?7
%但是计算波传播的时�?1?7�，会加�?1?72的光阑（�?1?73的光阑）以保证不会发�?1?7 wrap 的情�?1?7
%�?1?7以，推荐 1/sampling=kk 要小�?1?7 6
%由于6之后的数据是不准的，至少用现在方法，但是为了保证高频信息还有，就�?1?7要samping rate足够小�?1?7?1?7

%计算�?1?7高的空间rad值为多少。因为到6是有效的，所以乘以波长后，就是最高的mrad�?1?7
...
%计算中，为了防止wrap，计算过程�?1?7�易空间都加了半宽的光阑�?1?7
%为了保证到了较高空间还能有�?1?7�空间的分量�?1?75/sampling 是最高的倒空间频率， 0.5*(0.5*1/samping)*lambda=alpha mrad
% %�?1?7�?1?7 sampling=lambda*0.25/alpha ,注意化到埃和rad的单�?1?7其中�?1?7�?1?75�?1?7-0.5/sampling �?1?75/sampling的最高频�?1?7
% disp(strcat('Imaging sampling rate should be smaller than ', num2str(0.5*0.5*lambda*10/kmax),' A/pixel'));
%�?1?7�?1?7 sampling=lambda*0.25/alpha ,注意化到埃和rad的单�?1?7其中�?1?7�?1?75�?1?7-0.5/sampling
%�?1?75/sampling的最高频�?1?7现在取的�?1?73的光�?1?7
%disp(strcat('Imaging sampling rate should be smaller than ', num2str(0.5*2/3*lambda*10/kmax),' A/pixel'));
%�?1?75/sampling的最高频�?1?7现在取的�?1?72的光�?1?7
...
if get(handles.radiobutton9,'value')==1
    ...
    set(handles.text21, 'visible', 'on');   % aper的mrad�?1?7nm单位
    set(handles.text125, 'visible', 'off');
    set(handles.text134, 'visible', 'on');   % convergence & aperture的mrad�?1?7nm单位
...
if get(handles.radiobutton11,'value')==1
    ...
    set(handles.text21, 'visible', 'on');   % aper的mrad�?1?7nm单位
    set(handles.text125, 'visible', 'off');
    set(handles.text24, 'visible', 'off');   % convergence & aperture的mrad�?1?7nm单位
...
plot(x,-y,'r-','LineWidth',3)
%rad2=0.5*rad1;  %�?1?72倍aper的数值来画图，只是换算到mrad单位
...
me=9.1093807e-31;
c=2.9979251e+8;
tccpara.sampling=str2num(get(handles.edit75,'String'))*0.1; %  抽样�?1?7 %UNIT NM/PIXEL  %这里与EW_RECONSTRUCT不一样的单位
tccpara.mtotal=(get(handles.popupmenu4,'Value')-1)*2+1; %  总共有多少个离焦偏离，比如这个�?1?7�为5，就必须�?1?72 3 4 5个mytcc矩阵，其中第三张图无离焦偏离
...
tccpara.gmax=str2num(get(handles.edit_Ape,'String'))*0.1;   %换算到A的单�?1?7
 
resul=Gaussian_focal(tccpara.mtotal,tcccpara.yita,tccpara.lambda,tccpara.gmax); %为离焦图像所占�?1?7�图像的百分之几
...
tccpara.tiltx=tccpara.tilt/tccpara.lambda*cos(tccpara.phitilt/180*pi); %换算到�?1?7�易格式的单�?1?7 1/nm
...
function tccpara=readtccfromhandles_newTEM(hObject, handles, lambda)  %本次是根据TEM的参数来读取�?1?7
tccpara.lambda=lambda;
%if get(handles.checkbox_polar,'Value')==1  %必定是极坐标�?1?7
   %读取说明�?1?7%参�?1?7�Phil. Trans. R. Soc. A 2009, vol
%367:3755-3771;但是圈出来的系数在UM  1998 72 PP109-119以及UM 1996 64 249-264中，B2 S3 D4 B4
...
%配合 formular公式，该公式把所有的高阶像散形式都表现出来，且求得了对u和v方向的梯�?1?7
...
%参�?1?7�Phil. Trans. R. Soc. A 2009, vol
...
%参�?1?7�um 1996 vol 64:109-135，公�?1?7
...
%参�?1?7�Phil. Trans. R. Soc. A 2009, vol 367:3755-3771;公式2.2下方
%E_s_coh=exp(-para.alpha.*para.alpha/4/para.lambda/para.lambda.*gradient_W.*conj(gradient_W).*(2*pi*2*pi/para.lambda/para.lambda));  %空间相干�?1?7 这里para.alpha为半�?1?7gradient_kai实际上是书上的gradient_W，需要再乘以w*pi�?1?7
%本表式根据Phil. Trans. R. Soc. A 2009, vol
%367:3755-3771;公式2.1写出；注意gradient_W_u_v实际上是对W求导的�?1?7?1?7
%与参考um 1996 vol 64:109-135相比，该文献中的 kai=W/lambda，W为phil的文献变量，
%�?1?7以参考um 1996 vol 64:109-135，公�?1?7与本表示结果�?1?7�?1?7
E_f_coh=exp(-i*pi*(m-(para1.mtotal+1)/2)*para1.delta_yita*(u.^2+v.^2).*para1.lambda);  %色差   这里的u和v，是倒格矢量
%参�?1?7�um 1996 vol 64:109-135，公�?1?7,与本表式来源
t=exp(-i*kai);
% if m==1
...
%滤波
[sx,sy]=size(u);
if min(sx,sy)>2  %如果不是�?1?7维数据，就做带�?1?7�滤�?1?7
%   tlittle_all(:,:,m)=myaperture(tlittle_all(:,:,m),1/(para.sampling.*sx),1/(para.sampling.*sy),0,para.gmax,0,0,1);
else 
    
   tlittle_all(1,round(para1.gmax./para1.sampling):end,m)=0;  %�?1?7维函数的带�?1?7�，高频处都�?1?7�此时para.sampling是表示�?1?7�空间的间隔频率�?1?701nm_1的�?1?7�，固定下来�?1?7
end
end
...
default_val={ num2str(handles.dis_atomsize) };%默认的�?1?7?1?7
...
% --------------------------------------------------------------------
function Untitled_26_Callback(hObject, eventdata, handles)  %显示分层的图�?1?7
cd(handles.tempcd)  %进入上一次打�?1?7的目�?1?7
...
myvar.slicenum = paraslicing(1);  %这几个按钮都不要了�?1?7?1?7
...
cd(handles.saveresult)  %进入上一次打�?1?7的目�?1?7
...
guidata(hObject, handles);
% 
% NyO = round(handles.Oversampling*Ny);%Ny,Nx为图象尺�?1?7
...
if get(handles.popupmenu5,'value')==3  %如果采用lobato的系数，就不能分�?1?7
...
%导入波函数，�?1?7要尺寸信息�?1?7?1?7
[FileName,handles.batchs.PathName]=uigetfile({'*.dat','Data Only Format (*.dat)'});%打开后缀为txt的文�?1?7
if length(FileName)==1 & FileName(1)==0;  %如果选择cancal没打�?1?7文件的话，程序终�?1?7
...
%选择波函数后，波函数的图像要显示
    prompt={'Enter wave size: width (Cubic)';...
       'Input flag: Show wave(S) and Run simulation(R)'; 'Images'' number of ';'Process number';...
         ...
         'TOP-LEFT coordinate (x-y)';...  %�?1?7要一行内输入两个�?1?7
         'Intercept size (width & hight)' };   %�?1?7要一行内输入两个�?1?7
...
fid=fopen(strcat(handles.batchs.PathName,FileName),'r');  %读取波函数�?1?7�读石墨�?1?7  %1204把sampling改为0.1，不�?1?703，计算太慢，图像大小�?1?70
a=fread(fid,[handles.batchs.wavesx*2,handles.batchs.wavesy],'float');%
fclose(fid);
disimage=a(1:2:end,:)+sqrt(-1)*a(2:2:end,:);   %
mywave=disimage.';

if handles.batchs.showorrun == 'S'  %只是先看�?1?7下所选的区域是否合�?1?7��?1?7?1?7
    %把波函数显示出来，并且画出截取的区域位置
    figure;imshow(angle(disimage),[]); title('phase of wave');   %把波函数显示出来，并且要截取的区域，左上角用红圈，右下角用黄星表�?1?7
...
handles.lambda=1.0e+9*h.*c./sqrt(e*vol*1000*(2*me*c*c+e*vol*1000));  %计算波长，单位nm
handles.vol=vol*1000;
handles.sampling=str2num(get(handles.edit75,'string'));  %�?1?7/pixel  

%%绿色的尺寸，绿色倒格矢�?1?7�gx用于计算原子位置的，s2使用peng的参数计算原子势场分�?1?7
[gx_green,gy_green]=meshgrid((-sxy/2:(sxy/2-1))./(sxy*handles.sampling), ...
    (-sxy/2:(sxy/2-1))./(sxy*handles.sampling)); %REPRO单位�?1?7A)^2,不是(1/nm)^2
sx_green=gx_green/2;  % peng 的s参数
sy_green=gy_green/2;
s2_green=sx_green.^2+sy_green.^2;

mywave=fftshift(fft2(mywave));   %构�?1?7�波函数，如果从外界读取也可�?1?7到频率空�?1?7
...
[txtName,txtPathName]=uigetfile({'*.txt','TXT Only Format (*.txt)'});%打开后缀为txt的文�?1?7
if length(txtName)==1 & txtName(1)==0;  %如果选择cancal没打�?1?7文件的话，程序终�?1?7
    mycddir(handles.execd)
    return;
else
    %读取�?1?7有参数的数�?1?7?1?7
%newpara_part2.phiA1=newpara_part2.phiA1+(rand-0.5)*2*myrandrange.phiA1;
%%代码变成，所有的角度，也是在�?1?7定范围内，加减一个数值�?1?7�如果加�?1?70，就是正�?1?70绕一�?1?7
...
myrandrange.B2=allvalue(6);%10;%15;%16.4;%15; %units:nm ****  有加*的都�?1?709修改的范�?1?7
...
myrandrange.C5=allvalue(21);%1000000; %2000000;%3800000;%2000000; %units:nm ****  %其实也可以设置百分比�?1?7
...
% myrandrange.B2=10;%15;%16.4;%15; %units:nm ****  有加*的都�?1?709修改的范�?1?7
...
% myrandrange.C5=1000000; %2000000;%3800000;%2000000; %units:nm ****  %其实也可以设置百分比�?1?7
...
commpara=[para_part1.sampling, vol, ... %抽样率，kv单位的电�?1?7
        para_part1.gmax, yita, para_part1.mtotal, para_part1.alafa*1000, ...  %�?1?7高频率，focus spread，计算的高斯函数的个数， beam convergence
...
%         %newpara_part2.A1=round(newpara_part2.A1+(rand-0.5)*2*myrandrange.A1,1);  %测试�?1?7
%         newpara_part2.A1=newpara_part2.A1+(rand-0.5)*2*myrandrange.A1; %add at 20200820，实验像
%         newpara_part2.phiA1=A1_phi(round(rand*(90))+1); %实验�?1?7
%         %newpara_part2.phiA1=round(rand*180); %add at 20200820，测试像
%         %newpara_part2.A2=round(newpara_part2.A2+(rand-0.5)*2*myrandrange.A2,0);  %测试�?1?7
%         newpara_part2.A2=newpara_part2.A2+(rand-0.5)*2*myrandrange.A2; %add at 20200820，实验像
%         newpara_part2.phiA2=A2_phi(round(rand*(90))+1); %实验�?1?7
%         %newpara_part2.phiA2=round(rand*180); %add at 20200820
%         %newpara_part2.B2=round(newpara_part2.B2+(rand-0.5)*2*myrandrange.B2,0);  %加入随机数，并且保留小数点后1�?1?7
%         newpara_part2.B2=newpara_part2.B2+(rand-0.5)*2*myrandrange.B2; %add at 20200820,实验�?1?7
%         newpara_part2.phiB2=B2_phi(round(rand*(90))+1); %实验�?1?7
%         %newpara_part2.phiB2=round(rand*180); %add at 20200820，测试像
%         %newpara_part2.Cs=round(newpara_part2.Cs+(rand-0.5)*2*myrandrange.Cs,-2);  %加入随机数，并且保留小数点后1�?1?7
%         newpara_part2.Cs=newpara_part2.Cs+(rand-0.5)*2*myrandrange.Cs; %add at 20200820，实验像
%         %newpara_part2.A3=round(newpara_part2.A3+(rand-0.5)*2*myrandrange.A3,-2);  %加入随机数，并且保留小数点后1�?1?7
%         newpara_part2.A3=newpara_part2.A3+(rand-0.5)*2*myrandrange.A3; %add at 20200820，实验像
%         newpara_part2.phiA3=A3_phi(round(rand*(36))+1); %实验�?1?7
%         %newpara_part2.phiA3=round(rand*180); %add at 20200820，测试像
%         %newpara_part2.S3=round(newpara_part2.S3+(rand-0.5)*2*myrandrange.S3,-2);  %加入随机数，并且保留小数点后1�?1?7
%         newpara_part2.S3=newpara_part2.S3+(rand-0.5)*2*myrandrange.S3; %add at 20200820，实验像
%         newpara_part2.phiS3=S3_phi(round(rand*(36))+1); %实验�?1?7
%         %newpara_part2.phiS3=round(rand*180); %add at 20200820，测试像
% %不变的参数，就不加随�?1?7        %newpara_part2.A4=round(newpara_part2.A4+(rand-0.5)*2*myrandrange.A4,-3);  %加入随机数，并且保留小数点后1�?1?7
%         %newpara_part2.phiA4=A4_phi(round(rand*(36))+1);
%         %newpara_part2.D4=round(newpara_part2.D4+(rand-0.5)*2*myrandrange.D4,-3);  %加入随机数，并且保留小数点后1�?1?7
%         %newpara_part2.phiD4=D4_phi(round(rand*(36))+1);
%         %newpara_part2.B4=round(newpara_part2.B4+(rand-0.5)*2*myrandrange.B4,-3);  %加入随机数，并且保留小数点前4�?1?7
%         newpara_part2.B4=newpara_part2.B4+(rand-0.5)*2*myrandrange.B4; %add at 20200820，实验像
%         newpara_part2.phiB4=B4_phi(round(rand*(36))+1); %实验�?1?7
%         %newpara_part2.phiB4=round(rand*180); %测试�?1?7
%        %newpara_part2.A5=round(newpara_part2.A5+(rand-0.5)*2*myrandrange.A5,-5);  %加入随机数，并且保留小数点前4�?1?7
%       %newpara_part2.phiA5=A5_phi(round(rand*(36))+1);
%         %newpara_part2.C5=round(newpara_part2.C5+(rand-0.5)*2*myrandrange.C5,-5);  %加入随机数，并且保留小数点前4�?1?7
% 
%     
...
%    
%        % para_part2 = readtccfromhandles_newTEM(hObject, handles, para_part1.lambda);%如果测试面上的数据，就用这一�?1?7
%        % newpara_part2=para_part2; %如果测试面上的数据，就用这一�?1?7
%        
...
%         newpara_part2.S5/1000000, -newpara_part2.phiS5];  %通过tccread命令的参数，角度�?1?7要取�?1?7
%         
%         %allpara(:,i) = myallpara;
%        
