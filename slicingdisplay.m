...
handles.x_new(:,4) = handles.x_mysavenewz + top; %顶部加上一些空间，由用户定，这样可以保证划分原子时候，即使在某一层，也可以位于不同高度
eachthick=max((max(handles.x_new(:,4)) + bottom +0.00000001)./str2num(get(handles.edit3,'string')),0.0001);   % 单位变为埃 20201227 修改  %20220418多增加一点，避免底部没了
...
%%draw one more 再多画一层底面，黑色的，作为出射面；
surf(xx,yy,(i+1)*eachthick.*ones(size(xx)), zeros(size(xx)));
view([2 1 0])
hidden off
